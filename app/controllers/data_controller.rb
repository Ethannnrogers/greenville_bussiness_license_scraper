class DataController < ApplicationController
  def index
    ...
    resident_formatted = resident_data.drop(2).take(resident_data.length - 1).map do |r|
      row = r.map do |s|
        s.to_s.strip
      end
      Hash[headers.zip(row)]
    end
    non_resident_formatted = non_resident_data.drop(2).take(non_resident_data.length - 1).map do |r|
      row = r.map do |s|
        s.to_s.strip
      end
      Hash[headers.zip(row)]
